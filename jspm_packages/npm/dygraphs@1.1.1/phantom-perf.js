...
var config_file_template = "document.getElementById('points').value = (points);\n" + "document.getElementById('series').value = (series);\n" + "document.getElementById('repetitions').value = (repetitions);\n";
var RunBenchmark = function(points, series, repetitions, done_callback) {
  var page = require('webpage').create();
  fs.write(tmpfile, config_file_template.replace("(points)", points).replace("(series)", series).replace("(repetitions)", repetitions), "w");
...
RunBenchmark(points, series, repetitions, function(rep_times) {
  ...
  console.log(points + '/' + series + '/' + repetitions + ': ' + mean.toFixed(1) + '+/-' + std.toFixed(1) + ' ms (' + rep_times.join(', ') + ')');
  phantom.exit();
});
