  ...
  page.open(url, function(status) {
    ...
    results = page.evaluate(function() {
      var num_passing = 0,
          num_failing = 0;
      var failures = [];
      jstestdriver.attachListener({finish: function(tc, name, result, e) {
...
var LoadAllManualTests = function(totally_done_callback) {
  ...
  var loaded_page = make_barrier_closure(tasks.length, function() {
    setTimeout(function() {
      ...
      console.log('Successfully loaded ' + success + ' / ' + (success + failures) + ' test pages.');
