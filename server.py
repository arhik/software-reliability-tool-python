...
class Compute(tornado.web.RequestHandler):
	def post(self):
		...
		dataTrace2 = self.prepare1(lnL_values,x, endpoints)
		# print(dataTrace2)
		self.finish(json.dumps({"plotlyContour":dataTrace2, "plotlyPlot":dataTrace1}))
	...
	def prepare1(self,lnL_values,x, endpoints):
		...
		lnl_bmle = [lnL_values[i][0] for i in range(len(lnL_values))]
		lnl_amle = [n / (1 - math.exp(-i * (tn))) for i in lnl_bmle]
